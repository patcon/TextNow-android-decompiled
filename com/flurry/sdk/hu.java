package com.flurry.sdk;

import java.math.BigDecimal;
import java.math.BigInteger;

public abstract class hu extends hv
{
  static final BigDecimal u = new BigDecimal(-9223372036854775808L);
  static final BigDecimal v = new BigDecimal(9223372036854775807L);
  static final BigDecimal w = new BigDecimal(-9223372036854775808L);
  static final BigDecimal x = new BigDecimal(9223372036854775807L);
  protected long A;
  protected double B;
  protected BigInteger C;
  protected BigDecimal D;
  protected boolean E;
  protected int F;
  protected int G;
  protected int H;
  protected final ie d;
  protected boolean e;
  protected int f = 0;
  protected int g = 0;
  protected long h = 0L;
  protected int i = 1;
  protected int j = 0;
  protected long k = 0L;
  protected int l = 1;
  protected int m = 0;
  protected hw n;
  protected hm o;
  protected final sp p;
  protected char[] q = null;
  protected boolean r = false;
  protected sj s = null;
  protected byte[] t;
  protected int y = 0;
  protected int z;

  protected hu(ie paramie, int paramInt)
  {
    this.a = paramInt;
    this.d = paramie;
    this.p = paramie.d();
    this.n = hw.g();
  }

  private final void a(int paramInt1, char[] paramArrayOfChar, int paramInt2, int paramInt3)
  {
    String str = this.p.f();
    try
    {
      if (ii.a(paramArrayOfChar, paramInt2, paramInt3, this.E))
      {
        this.A = Long.parseLong(str);
        this.y = 2;
        return;
      }
      this.C = new BigInteger(str);
      this.y = 4;
      return;
    }
    catch (NumberFormatException localNumberFormatException)
    {
      a("Malformed numeric value '" + str + "'", localNumberFormatException);
    }
  }

  private final void d(int paramInt)
  {
    if (paramInt == 16);
    try
    {
      this.D = this.p.h();
      this.y = 16;
      return;
      this.B = this.p.i();
      this.y = 8;
      return;
    }
    catch (NumberFormatException localNumberFormatException)
    {
      a("Malformed numeric value '" + this.p.f() + "'", localNumberFormatException);
    }
  }

  public final long A()
  {
    return this.k;
  }

  public final int B()
  {
    return this.l;
  }

  public final int C()
  {
    int i1 = this.m;
    if (i1 < 0)
      return i1;
    return i1 + 1;
  }

  protected final void D()
  {
    if (!E())
      S();
  }

  protected abstract boolean E();

  protected abstract void F();

  protected void G()
  {
    this.p.a();
    char[] arrayOfChar = this.q;
    if (arrayOfChar != null)
    {
      this.q = null;
      this.d.c(arrayOfChar);
    }
  }

  protected void H()
  {
    if (!this.n.b())
      c(": expected close marker for " + this.n.d() + " (from " + this.n.a(this.d.a()) + ")");
  }

  public sj I()
  {
    if (this.s == null)
      this.s = new sj();
    while (true)
    {
      return this.s;
      this.s.a();
    }
  }

  protected void J()
  {
    if ((0x2 & this.y) != 0)
    {
      int i1 = (int)this.A;
      if (i1 != this.A)
        d("Numeric value (" + k() + ") out of range of int");
      this.z = i1;
    }
    while (true)
    {
      this.y = (0x1 | this.y);
      return;
      if ((0x4 & this.y) != 0)
      {
        this.z = this.C.intValue();
      }
      else if ((0x8 & this.y) != 0)
      {
        if ((this.B < -2147483648.0D) || (this.B > 2147483647.0D))
          O();
        this.z = ((int)this.B);
      }
      else if ((0x10 & this.y) != 0)
      {
        if ((w.compareTo(this.D) > 0) || (x.compareTo(this.D) < 0))
          O();
        this.z = this.D.intValue();
      }
      else
      {
        U();
      }
    }
  }

  protected void K()
  {
    if ((0x1 & this.y) != 0)
      this.A = this.z;
    while (true)
    {
      this.y = (0x2 | this.y);
      return;
      if ((0x4 & this.y) != 0)
      {
        this.A = this.C.longValue();
      }
      else if ((0x8 & this.y) != 0)
      {
        if ((this.B < -9.223372036854776E+18D) || (this.B > 9.223372036854776E+18D))
          P();
        this.A = (()this.B);
      }
      else if ((0x10 & this.y) != 0)
      {
        if ((u.compareTo(this.D) > 0) || (v.compareTo(this.D) < 0))
          P();
        this.A = this.D.longValue();
      }
      else
      {
        U();
      }
    }
  }

  protected void L()
  {
    if ((0x10 & this.y) != 0)
      this.C = this.D.toBigInteger();
    while (true)
    {
      this.y = (0x4 | this.y);
      return;
      if ((0x2 & this.y) != 0)
        this.C = BigInteger.valueOf(this.A);
      else if ((0x1 & this.y) != 0)
        this.C = BigInteger.valueOf(this.z);
      else if ((0x8 & this.y) != 0)
        this.C = BigDecimal.valueOf(this.B).toBigInteger();
      else
        U();
    }
  }

  protected void M()
  {
    if ((0x10 & this.y) != 0)
      this.B = this.D.doubleValue();
    while (true)
    {
      this.y = (0x8 | this.y);
      return;
      if ((0x4 & this.y) != 0)
        this.B = this.C.doubleValue();
      else if ((0x2 & this.y) != 0)
        this.B = this.A;
      else if ((0x1 & this.y) != 0)
        this.B = this.z;
      else
        U();
    }
  }

  protected void N()
  {
    if ((0x8 & this.y) != 0)
      this.D = new BigDecimal(k());
    while (true)
    {
      this.y = (0x10 | this.y);
      return;
      if ((0x4 & this.y) != 0)
        this.D = new BigDecimal(this.C);
      else if ((0x2 & this.y) != 0)
        this.D = BigDecimal.valueOf(this.A);
      else if ((0x1 & this.y) != 0)
        this.D = BigDecimal.valueOf(this.z);
      else
        U();
    }
  }

  protected void O()
  {
    d("Numeric value (" + k() + ") out of range of int (" + -2147483648 + " - " + 2147483647 + ")");
  }

  protected void P()
  {
    d("Numeric value (" + k() + ") out of range of long (" + -9223372036854775808L + " - " + 9223372036854775807L + ")");
  }

  protected char Q()
  {
    throw new UnsupportedOperationException();
  }

  protected final int a(ha paramha, char paramChar, int paramInt)
  {
    if (paramChar != '\\')
      throw b(paramha, paramChar, paramInt);
    char c = Q();
    int i1;
    if ((c <= ' ') && (paramInt == 0))
      i1 = -1;
    do
    {
      return i1;
      i1 = paramha.b(c);
    }
    while (i1 >= 0);
    throw b(paramha, c, paramInt);
  }

  protected final int a(ha paramha, int paramInt1, int paramInt2)
  {
    if (paramInt1 != 92)
      throw b(paramha, paramInt1, paramInt2);
    int i1 = Q();
    int i2;
    if ((i1 <= 32) && (paramInt2 == 0))
      i2 = -1;
    do
    {
      return i2;
      i2 = paramha.b(i1);
    }
    while (i2 >= 0);
    throw b(paramha, i1, paramInt2);
  }

  protected final hm a(String paramString, double paramDouble)
  {
    this.p.a(paramString);
    this.B = paramDouble;
    this.y = 8;
    return hm.j;
  }

  protected final hm a(boolean paramBoolean, int paramInt)
  {
    this.E = paramBoolean;
    this.F = paramInt;
    this.G = 0;
    this.H = 0;
    this.y = 0;
    return hm.i;
  }

  protected final hm a(boolean paramBoolean, int paramInt1, int paramInt2, int paramInt3)
  {
    if ((paramInt2 <= 0) && (paramInt3 <= 0))
      return a(paramBoolean, paramInt1);
    return b(paramBoolean, paramInt1, paramInt2, paramInt3);
  }

  protected IllegalArgumentException a(ha paramha, int paramInt1, int paramInt2, String paramString)
  {
    String str;
    if (paramInt1 <= 32)
      str = "Illegal white space character (code 0x" + Integer.toHexString(paramInt1) + ") as character #" + (paramInt2 + 1) + " of 4-char base64 unit: can only used between units";
    while (true)
    {
      if (paramString != null)
        str = str + ": " + paramString;
      return new IllegalArgumentException(str);
      if (paramha.a(paramInt1))
        str = "Unexpected padding character ('" + paramha.b() + "') as character #" + (paramInt2 + 1) + " of 4-char base64 unit: padding only legal as 3rd or 4th character";
      else if ((!Character.isDefined(paramInt1)) || (Character.isISOControl(paramInt1)))
        str = "Illegal character (code 0x" + Integer.toHexString(paramInt1) + ") in base64 content";
      else
        str = "Illegal character '" + (char)paramInt1 + "' (code 0x" + Integer.toHexString(paramInt1) + ") in base64 content";
    }
  }

  protected void a(int paramInt)
  {
    if (this.b == hm.i)
    {
      char[] arrayOfChar = this.p.e();
      int i1 = this.p.d();
      int i2 = this.F;
      if (this.E)
        i1++;
      if (i2 <= 9)
      {
        int i3 = ii.a(arrayOfChar, i1, i2);
        if (this.E)
          i3 = -i3;
        this.z = i3;
        this.y = 1;
        return;
      }
      if (i2 <= 18)
      {
        long l1 = ii.b(arrayOfChar, i1, i2);
        if (this.E)
          l1 = -l1;
        if (i2 == 10)
          if (this.E)
          {
            if (l1 >= -2147483648L)
            {
              this.z = ((int)l1);
              this.y = 1;
            }
          }
          else if (l1 <= 2147483647L)
          {
            this.z = ((int)l1);
            this.y = 1;
            return;
          }
        this.A = l1;
        this.y = 2;
        return;
      }
      a(paramInt, arrayOfChar, i1, i2);
      return;
    }
    if (this.b == hm.j)
    {
      d(paramInt);
      return;
    }
    d("Current token (" + this.b + ") not numeric, can not use numeric value accessors");
  }

  protected void a(int paramInt, char paramChar)
  {
    String str = "" + this.n.a(this.d.a());
    d("Unexpected close marker '" + (char)paramInt + "': expected '" + paramChar + "' (for " + this.n.d() + " starting at " + str + ")");
  }

  protected void a(int paramInt, String paramString)
  {
    String str = "Unexpected character (" + c(paramInt) + ") in numeric value";
    if (paramString != null)
      str = str + ": " + paramString;
    d(str);
  }

  protected final hm b(boolean paramBoolean, int paramInt1, int paramInt2, int paramInt3)
  {
    this.E = paramBoolean;
    this.F = paramInt1;
    this.G = paramInt2;
    this.H = paramInt3;
    this.y = 0;
    return hm.j;
  }

  protected IllegalArgumentException b(ha paramha, int paramInt1, int paramInt2)
  {
    return a(paramha, paramInt1, paramInt2, null);
  }

  protected void b(String paramString)
  {
    d("Invalid numeric value: " + paramString);
  }

  public void close()
  {
    if (!this.e)
      this.e = true;
    try
    {
      F();
      return;
    }
    finally
    {
      G();
    }
  }

  public String g()
  {
    if ((this.b == hm.b) || (this.b == hm.d))
      return this.n.i().h();
    return this.n.h();
  }

  public hg h()
  {
    return new hg(this.d.a(), A(), B(), C());
  }

  public hg i()
  {
    int i1 = 1 + (this.f - this.j);
    return new hg(this.d.a(), this.h + this.f - 1L, this.i, i1);
  }

  public boolean o()
  {
    if (this.b == hm.h)
      return true;
    if (this.b == hm.f)
      return this.r;
    return false;
  }

  public Number p()
  {
    if (this.y == 0)
      a(0);
    if (this.b == hm.i)
    {
      if ((0x1 & this.y) != 0)
        return Integer.valueOf(this.z);
      if ((0x2 & this.y) != 0)
        return Long.valueOf(this.A);
      if ((0x4 & this.y) != 0)
        return this.C;
      return this.D;
    }
    if ((0x10 & this.y) != 0)
      return this.D;
    if ((0x8 & this.y) == 0)
      U();
    return Double.valueOf(this.B);
  }

  public hj.b q()
  {
    if (this.y == 0)
      a(0);
    if (this.b == hm.i)
    {
      if ((0x1 & this.y) != 0)
        return hj.b.a;
      if ((0x2 & this.y) != 0)
        return hj.b.b;
      return hj.b.c;
    }
    if ((0x10 & this.y) != 0)
      return hj.b.f;
    return hj.b.e;
  }

  public int t()
  {
    if ((0x1 & this.y) == 0)
    {
      if (this.y == 0)
        a(1);
      if ((0x1 & this.y) == 0)
        J();
    }
    return this.z;
  }

  public long u()
  {
    if ((0x2 & this.y) == 0)
    {
      if (this.y == 0)
        a(2);
      if ((0x2 & this.y) == 0)
        K();
    }
    return this.A;
  }

  public BigInteger v()
  {
    if ((0x4 & this.y) == 0)
    {
      if (this.y == 0)
        a(4);
      if ((0x4 & this.y) == 0)
        L();
    }
    return this.C;
  }

  public float w()
  {
    return (float)x();
  }

  public double x()
  {
    if ((0x8 & this.y) == 0)
    {
      if (this.y == 0)
        a(8);
      if ((0x8 & this.y) == 0)
        M();
    }
    return this.B;
  }

  public BigDecimal y()
  {
    if ((0x10 & this.y) == 0)
    {
      if (this.y == 0)
        a(16);
      if ((0x10 & this.y) == 0)
        N();
    }
    return this.D;
  }
}

/* Location:           /home/patcon/Downloads/com.enflick.android.TextNow-dex2jar.jar
 * Qualified Name:     com.flurry.sdk.hu
 * JD-Core Version:    0.6.2
 */