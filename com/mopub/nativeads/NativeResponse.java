...
public class NativeResponse
{
  ...
  private void openClickDestinationUrl(View paramView)
  {
    if (getClickDestinationUrl() == null)
      return;
    SpinningProgressView localSpinningProgressView = null;
    if (paramView != null)
    {
      localSpinningProgressView = new SpinningProgressView(this.mContext);
      localSpinningProgressView.addToRoot(paramView);
    }
    String[] arrayOfString = new String[1];
    arrayOfString[0] = getClickDestinationUrl();
    Iterator localIterator = Arrays.asList(arrayOfString).iterator();
    NativeResponse.ClickDestinationUrlResolutionListener localClickDestinationUrlResolutionListener = new NativeResponse.ClickDestinationUrlResolutionListener(this.mContext, localIterator, localSpinningProgressView);
    UrlResolutionTask.getResolvedUrl((String)localIterator.next(), localClickDestinationUrlResolutionListener);
  }
  ...
  {
    Object localObject = getExtra(paramString);
    if ((localObject != null) && ((localObject instanceof String)))
      ImageViewService.loadImageView((String)localObject, paramImageView);
  }

  public void loadIconImage(ImageView paramImageView)
  {
    ImageViewService.loadImageView(getIconImageUrl(), paramImageView);
  }

  public void loadMainImage(ImageView paramImageView)
  {
    ImageViewService.loadImageView(getMainImageUrl(), paramImageView);
  }
